...
- **OpenCode Go** is a flat-fee subscription at `https://opencode.ai/zen/go/v1`. Jin routes each model to the documented endpoint, including Grok 4.6 and GPT-5.6 Luna over the Responses API (Grok 4.6: always-on low/medium/high/xhigh reasoning, vision, sampling), plus Muse Spark 1.2 and Muse Spark 1.2 Contributor over the Responses API (always-on reasoning with a visible effort control, vision, native PDF). Contributor-tier prompts may be used to train Meta models.
- **Ramp Router** is a gateway at `https://api.router.com/v1` fronting Anthropic, OpenAI, xAI, and Fireworks-hosted open models on one key. It is Responses-only — there is no Chat Completions endpoint — so Jin sends every model to `POST /v1/responses`. Model IDs are account-scoped and differ from the labels in Router's docs table (`claude-opus-5`, `accounts/fireworks/models/kimi-k3`), so use **Fetch Models**: Jin reads Router's per-model limits, input modalities, and reasoning bands straight off `/v1/models`. Web search is available on the OpenAI-served models; Router has no file-upload API, so PDFs are sent as extracted text.
...
- **Makora** is an OpenAI-compatible inference API at `https://inference.makora.com/v1`. Jin seeds the current makora.com lineup (Kimi K3, DeepSeek V4 Flash 0731, GLM 5.2 FP8/NVFP4, Gemma 4 26B A4B) and rewrites vLLM thinking parameters (`chat_template_kwargs`, `include_reasoning`) so reasoning actually turns on. Fetch Models pulls the live `/v1/models` catalog.
