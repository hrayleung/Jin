import Foundation
import Observation

/// Lives outside ChatView's `@State` so that mutating editor text does not
/// invalidate ChatView's body. The composer and the in-bubble user-message
/// editor each own an instance. Views that need to react to keystrokes
/// (binding to the editor, computing `canSendDraft`, watching for slash
/// commands) observe this store directly; ChatView itself only passes the
/// reference down and reads `text` from non-body contexts.
@Observable
@MainActor
final class ComposerTextStore {
    var text: String = ""
}
