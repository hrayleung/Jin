...
extension ProviderFormSupport {
    ...
    static func credentialKind(for providerType: ProviderType) -> CredentialKind {
        ...
        case .githubCopilot, .openai, .openaiWebSocket, .openaiCompatible, .cloudflareAIGateway, .vercelAIGateway, .openrouter,
             ...
             .fireworks, .cerebras, .sambanova, .databricks, .morphllm, .opencodeGo, .router, .runinfra, .gemini, .zyphra, .makora, .meta, .kimiForCoding:
            return .apiKey
        case .modal:
    ...
    static func providerDetailsText(for providerType: ProviderType) -> String? {
        ...
        case .kimiForCoding:
            return "Anthropic-compatible endpoint at `https://api.kimi.com/coding` (requests go to `/v1/messages`). Use an API key from the Kimi Code Console. Models: `k3`, `k3-256k`, `kimi-for-coding`, `kimi-for-coding-highspeed`."
        case .databricks:
        ...
        case .runinfra:
            return "RunInfra Model APIs at `https://api.runinfra.ai/v1` (workspace key from runinfra.ai/inference-api, typically `rp_…`). OpenAI-compatible chat completions; live ids include `deepseek-v4-flash`, `deepseek-v4-pro`, `qwen3-8-27b`, `qwen3-8-flash-next`, `qwen3-8-2-4t-a95b`, `nemotron-3-5-lightning-30b`, `ornith-1-5-35b`, and `glm-5-3-flash`. Reasoning uses top-level `reasoning_effort` (including `none` except on Qwen3.8 2.4T). Only `qwen3-8-27b` currently accepts image parts."
        case .router:
