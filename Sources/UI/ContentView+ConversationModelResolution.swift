...
extension ContentView {
    ...
    func defaultModelID(for providerID: String) -> String {
        ...
        guard !models.isEmpty else {
            return Self.absoluteFallbackModelID(for: providerID)
        }
        ...
        if providerID == "anthropic", let sonnet5 = models.first(where: { $0.id == "claude-sonnet-5" }) {
            return sonnet5.id
        }
    ...
    /// Absolute per-provider fallback model IDs, used when a provider exposes no
    /// selectable models (and as the defensive tail when the preference ladder
    /// and `models.first` both miss). Single source of truth so the two fallback
    /// paths cannot drift apart.
    private static func absoluteFallbackModelID(for providerID: String) -> String {
        absoluteFallbackModelIDs[providerID] ?? "gpt-5.2"
    }
