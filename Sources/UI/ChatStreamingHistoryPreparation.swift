...
extension ChatStreamingOrchestrator {
    static func prepareHistory(from ctx: SessionContext) -> [Message] {
        var history: [Message]
        if let prebuilt = ctx.prebuiltHistory {
            // Already ordered domain messages from the render cache — no
            // SwiftData fault + JSON decode round-trip on the send path.
            history = prebuilt
        } else {
            let decoder = JSONDecoder()
            history = ctx.messageSnapshots
                .sorted { lhs, rhs in
