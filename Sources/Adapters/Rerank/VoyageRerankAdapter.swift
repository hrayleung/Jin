...
actor VoyageRerankAdapter: RerankProviderAdapter {
    ...
    func fetchAvailableModels() async throws -> [RerankModelInfo] {
        [
            RerankModelInfo(id: "rerank-2", name: "Rerank 2", maxInputTokens: 16000),
            RerankModelInfo(id: "rerank-2-lite", name: "Rerank 2 Lite", maxInputTokens: 4000)
        ]
    }
    ...
    private func parseResponse(_ data: Data) throws -> RerankResponse {
        ...
        guard let results = json?["data"] as? [[String: Any]] else {
            throw LLMError.decodingError(message: "Invalid Voyage rerank response format")
        }
