var _codeBlockId = 0;
var _codeBlockSrc = {};
var _codeBlockState = {};
var _codeBlockDisplayMode = 'expanded';
var _codeBlockCollapseLineThreshold = 25;
var _latestStableBase64 = '';
var _stableRenderTicket = 0;
var _latestStreamBase64 = '';
var _streamRenderTicket = 0;
var _latestStableText = '';
var _latestStreamText = '';
var _prismReady = false;
var _prismRuntimeLoadPromise = null;
var _mathRuntimeLoadPromise = null;
var _mathStyleLoadPromise = null;
var _mathReady = false;
var _mathStyleReady = false;
var _mermaidRuntimeLoadPromise = null;
var _mermaidReadyPromise = null;
var MAX_HIGHLIGHT_CODE_LENGTH = 24000;
var MAX_HIGHLIGHT_CODE_LINES = 700;
var _selectionMessageID = '';
var _selectionContextThreadID = '';
var _selectionAnchorID = '';
var _persistedHighlights = [];
var _selectionSyncTimer = null;

function contentRoot() {
    return document.getElementById('content');
}

function isSelectableChromeElement(element, root) {
    if (!element || element === root) return false;
    if (element.hidden || element.getAttribute('aria-hidden') === 'true') return true;
    if (
        element.closest('.code-header, .line-numbers-gutter, .code-expand-bar, .code-actions, button') &&
        !element.closest('mark[data-jin-highlight-id]')
    ) {
        return true;
    }
    var style = window.getComputedStyle ? window.getComputedStyle(element) : null;
    if (style && (style.display === 'none' || style.visibility === 'hidden' || style.userSelect === 'none' || style.webkitUserSelect === 'none')) {
        return true;
    }
    return false;
}

function isSelectableTextNode(node, root) {
    if (!node || !node.nodeValue || !node.nodeValue.length) return false;
    var current = node.parentNode;
    while (current && current !== root) {
        if (current.nodeType === Node.ELEMENT_NODE) {
            var tagName = String(current.tagName || '');
            if (tagName === 'SCRIPT' || tagName === 'STYLE') return false;
            if (isSelectableChromeElement(current, root)) return false;
        }
        current = current.parentNode;
    }
    var parent = node.parentNode;
    if (parent && parent.nodeType === Node.ELEMENT_NODE) {
        if (typeof parent.getClientRects === 'function' && parent.getClientRects().length === 0) {
            return false;
        }
    }
    return true;
}

function collectTextNodes(root) {
    var nodes = [];
    if (!root) return nodes;
    var walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
        acceptNode: function(node) {
            return isSelectableTextNode(node, root)
                ? NodeFilter.FILTER_ACCEPT
                : NodeFilter.FILTER_REJECT;
        }
    });
    var current;
    while ((current = walker.nextNode())) {
        nodes.push(current);
    }
    return nodes;
}

function totalTextLength(root) {
    var nodes = collectTextNodes(root);
    var total = 0;
    for (var i = 0; i < nodes.length; i += 1) {
        total += nodes[i].nodeValue.length;
    }
    return total;
}

function selectableTextContent(root) {
    var nodes = collectTextNodes(root);
    var parts = [];
    for (var i = 0; i < nodes.length; i += 1) {
        parts.push(nodes[i].nodeValue);
    }
    return parts.join('');
}

function normalizeBoundaryPosition(root, container, offset, isStart) {
    if (!container) return null;
    if (container.nodeType === Node.TEXT_NODE) {
        return { node: container, offset: offset };
    }

    var nodes = collectTextNodes(root);
    if (!nodes.length) return null;

    var referenceNode = null;
    if (container.nodeType === Node.ELEMENT_NODE) {
        referenceNode = container.childNodes[offset] || null;
        if (referenceNode && referenceNode.nodeType !== Node.TEXT_NODE) {
            if (isStart) {
                var walker = document.createTreeWalker(referenceNode, NodeFilter.SHOW_TEXT, null);
                referenceNode = walker.nextNode();
            } else {
                var lastText = null;
                var reverseWalker = document.createTreeWalker(referenceNode, NodeFilter.SHOW_TEXT, null);
                var current;
                while ((current = reverseWalker.nextNode())) lastText = current;
                referenceNode = lastText;
            }
        }
    }

    if (referenceNode && referenceNode.nodeType === Node.TEXT_NODE) {
        return { node: referenceNode, offset: isStart ? 0 : referenceNode.nodeValue.length };
    }

    var candidate = isStart ? nodes[0] : nodes[nodes.length - 1];
    if (container.nodeType === Node.ELEMENT_NODE) {
        var foundIndex = -1;
        for (var i = 0; i < nodes.length; i += 1) {
            if (container.contains(nodes[i])) {
                foundIndex = i;
                break;
            }
        }
        if (foundIndex >= 0) {
            candidate = isStart ? nodes[foundIndex] : nodes[foundIndex];
        }
    }
    return { node: candidate, offset: isStart ? 0 : candidate.nodeValue.length };
}

function locateTextPosition(root, offset) {
    var nodes = collectTextNodes(root);
    if (!nodes.length) return null;
    var remaining = Math.max(0, offset || 0);
    for (var i = 0; i < nodes.length; i += 1) {
        var node = nodes[i];
        var length = node.nodeValue.length;
        if (remaining <= length) {
            return { node: node, offset: remaining };
        }
        remaining -= length;
    }
    var lastNode = nodes[nodes.length - 1];
    return { node: lastNode, offset: lastNode.nodeValue.length };
}

function unwrapHighlightMarks(root) {
    if (!root) return;
    var marks = Array.from(root.querySelectorAll('mark[data-jin-highlight-id]'));
    marks.forEach(function(mark) {
        var parent = mark.parentNode;
        if (!parent) return;
        while (mark.firstChild) {
            parent.insertBefore(mark.firstChild, mark);
        }
        parent.removeChild(mark);
        parent.normalize();
    });
}

function commonPrefixLength(lhs, rhs) {
    var max = Math.min(lhs.length, rhs.length);
    var count = 0;
    while (count < max && lhs.charAt(count) === rhs.charAt(count)) {
        count += 1;
    }
    return count;
}

function commonSuffixLength(lhs, rhs) {
    var lhsLength = lhs.length;
    var rhsLength = rhs.length;
    var count = 0;
    while (
        count < lhsLength &&
        count < rhsLength &&
        lhs.charAt(lhsLength - count - 1) === rhs.charAt(rhsLength - count - 1)
    ) {
        count += 1;
    }
    return count;
}

function resolveHighlightOffsets(rootText, highlight) {
    var start = Math.max(0, parseInt(highlight.startOffset, 10) || 0);
    var end = Math.max(start, parseInt(highlight.endOffset, 10) || start);
    var selectedText = String(highlight.selectedText || '');
    if (!selectedText) {
        return { start: start, end: end };
    }

    if (rootText.slice(start, end) === selectedText) {
        return { start: start, end: end };
    }

    var prefixContext = String(highlight.prefixContext || '');
    var suffixContext = String(highlight.suffixContext || '');
    var searchIndex = 0;
    var bestMatch = null;
    var matchCount = 0;

    while (searchIndex <= rootText.length) {
        var foundIndex = rootText.indexOf(selectedText, searchIndex);
        if (foundIndex < 0) break;

        matchCount += 1;
        var candidateStart = foundIndex;
        var candidateEnd = foundIndex + selectedText.length;
        var score = 0;

        if (prefixContext) {
            var actualPrefix = rootText.slice(Math.max(0, candidateStart - prefixContext.length), candidateStart);
            score += commonSuffixLength(actualPrefix, prefixContext) * 2;
            if (actualPrefix === prefixContext) score += 48;
        }

        if (suffixContext) {
            var actualSuffix = rootText.slice(candidateEnd, Math.min(rootText.length, candidateEnd + suffixContext.length));
            score += commonPrefixLength(actualSuffix, suffixContext) * 2;
            if (actualSuffix === suffixContext) score += 48;
        }

        var distanceFromStored = Math.abs(candidateStart - start);
        score -= Math.min(distanceFromStored, 24);

        if (!bestMatch || score > bestMatch.score || (score === bestMatch.score && distanceFromStored < bestMatch.distance)) {
            bestMatch = {
                start: candidateStart,
                end: candidateEnd,
                score: score,
                distance: distanceFromStored
            };
        }

        searchIndex = foundIndex + Math.max(1, selectedText.length);
    }

    if (bestMatch && (bestMatch.score > 0 || matchCount === 1)) {
        return { start: bestMatch.start, end: bestMatch.end };
    }

    return { start: start, end: end };
}

function applyHighlightRange(root, highlight) {
    if (!root || !highlight) return;
    var rootText = selectableTextContent(root);
    var resolvedOffsets = resolveHighlightOffsets(rootText, highlight);
    var start = resolvedOffsets.start;
    var end = Math.max(start, resolvedOffsets.end);
    if (end <= start) return;

    var startPos = locateTextPosition(root, start);
    var endPos = locateTextPosition(root, end);
    if (!startPos || !endPos) return;

    var range = document.createRange();
    range.setStart(startPos.node, Math.min(startPos.offset, startPos.node.nodeValue.length));
    range.setEnd(endPos.node, Math.min(endPos.offset, endPos.node.nodeValue.length));
    if (range.collapsed) return;

    var mark = document.createElement('mark');
    mark.setAttribute('data-jin-highlight-id', highlight.id);
    mark.setAttribute('data-jin-highlight-style', highlight.colorStyle || 'readerYellow');

    try {
        var fragment = range.extractContents();
        mark.appendChild(fragment);
        range.insertNode(mark);
    } catch (error) {
        console.warn('[JinMarkdown] failed to apply persisted highlight', error);
    }
}

function applyPersistedHighlights() {
    var root = contentRoot();
    if (!root) return;
    unwrapHighlightMarks(root);
    var highlights = Array.isArray(_persistedHighlights) ? _persistedHighlights.slice() : [];
    highlights.sort(function(a, b) {
        return (parseInt(b.startOffset, 10) || 0) - (parseInt(a.startOffset, 10) || 0);
    });
    highlights.forEach(function(highlight) {
        applyHighlightRange(root, highlight);
    });
}

function rangeToOffsets(root, range) {
    var nodes = collectTextNodes(root);
    var normalizedStart = normalizeBoundaryPosition(root, range.startContainer, range.startOffset, true);
    var normalizedEnd = normalizeBoundaryPosition(root, range.endContainer, range.endOffset, false);
    if (!normalizedStart || !normalizedEnd) {
        return { start: 0, end: 0 };
    }
    var start = 0;
    var end = 0;
    var foundStart = false;
    var foundEnd = false;

    for (var i = 0; i < nodes.length; i += 1) {
        var node = nodes[i];
        var length = node.nodeValue.length;
        if (!foundStart) {
            if (node === normalizedStart.node) {
                start += normalizedStart.offset;
                foundStart = true;
            } else {
                start += length;
            }
        }
        if (!foundEnd) {
            if (node === normalizedEnd.node) {
                end += normalizedEnd.offset;
                foundEnd = true;
            } else {
                end += length;
            }
        }
        if (foundStart && foundEnd) break;
    }

    return {
        start: foundStart ? start : 0,
        end: foundEnd ? end : 0
    };
}

function buildSelectionSnapshot() {
    var root = contentRoot();
    if (!root) return null;
    var selection = window.getSelection ? window.getSelection() : null;
    if (!selection || selection.rangeCount === 0 || selection.isCollapsed) return null;
    var range = selection.getRangeAt(0);
    if (!root.contains(range.commonAncestorContainer)) return null;

    var offsets = rangeToOffsets(root, range);
    var start = Math.max(0, offsets.start);
    var end = Math.max(start, offsets.end);
    var selectedText = String(selection.toString() || '');
    if (!selectedText.trim()) return null;

    var rootText = selectableTextContent(root);
    var matchingHighlightIDs = (_persistedHighlights || [])
        .filter(function(highlight) {
            return (parseInt(highlight.startOffset, 10) || 0) === start
                && (parseInt(highlight.endOffset, 10) || 0) === end;
        })
        .map(function(highlight) { return String(highlight.id || ''); })
        .filter(Boolean);

    return {
        messageID: _selectionMessageID,
        contextThreadID: _selectionContextThreadID,
        anchorID: _selectionAnchorID,
        selectedText: selectedText,
        prefixContext: rootText.slice(Math.max(0, start - 48), start),
        suffixContext: rootText.slice(end, Math.min(rootText.length, end + 48)),
        startOffset: start,
        endOffset: end,
        matchingHighlightIDs: matchingHighlightIDs
    };
}

function postSelectionSnapshot() {
    if (!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.selectionChanged)) {
        return;
    }
    window.webkit.messageHandlers.selectionChanged.postMessage(buildSelectionSnapshot());
}

function scheduleSelectionSnapshotPost() {
    if (_selectionSyncTimer) clearTimeout(_selectionSyncTimer);
    _selectionSyncTimer = setTimeout(function() {
        _selectionSyncTimer = null;
        postSelectionSnapshot();
    }, 0);
}

function setSelectionContext(messageID, contextThreadID, anchorID) {
    _selectionMessageID = String(messageID || '');
    _selectionContextThreadID = String(contextThreadID || '');
    _selectionAnchorID = String(anchorID || '');
}

function setPersistedHighlights(highlights) {
    _persistedHighlights = Array.isArray(highlights) ? highlights : [];
    applyPersistedHighlights();
    scheduleSelectionSnapshotPost();
    scheduleHeightReport();
}

function escapeHTML(value) {
    return String(value)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/\"/g, '&quot;')
        .replace(/'/g, '&#039;');
}

function resolveMermaidFromGlobal() {
    if (!window.mermaid && window.__esbuild_esm_mermaid_nm && window.__esbuild_esm_mermaid_nm.mermaid) {
        window.mermaid = window.__esbuild_esm_mermaid_nm.mermaid;
    }
}

function loadPrismRuntimeScript() {
    if (window.JinPrism && typeof window.JinPrism.highlightCode === 'function') {
        return Promise.resolve(true);
    }
    if (_prismRuntimeLoadPromise) {
        return _prismRuntimeLoadPromise;
    }
    _prismRuntimeLoadPromise = new Promise(function(resolve) {
        var script = document.createElement('script');
        script.src = 'markdown-prism-runtime.js';
        script.async = true;
        script.onload = function() {
            _prismReady = !!(window.JinPrism && typeof window.JinPrism.highlightCode === 'function');
            resolve(_prismReady);
        };
        script.onerror = function(err) {
            console.warn('[JinPrism] failed to load runtime script', err);
            resolve(false);
        };
        document.head.appendChild(script);
    });
    return _prismRuntimeLoadPromise;
}

function loadScriptOnce(src, warningLabel) {
    return new Promise(function(resolve) {
        var script = document.createElement('script');
        script.src = src;
        script.async = true;
        script.onload = function() { resolve(true); };
        script.onerror = function(err) {
            console.warn('[' + warningLabel + '] failed to load runtime script', err);
            resolve(false);
        };
        document.head.appendChild(script);
    });
}

function ensureMathStylesheet() {
    if (_mathStyleReady) return Promise.resolve(true);
    if (_mathStyleLoadPromise) return _mathStyleLoadPromise;

    _mathStyleLoadPromise = new Promise(function(resolve) {
        var existing = document.getElementById('jin-katex-style');
        if (existing) {
            _mathStyleReady = true;
            resolve(true);
            return;
        }

        var link = document.createElement('link');
        link.id = 'jin-katex-style';
        link.rel = 'stylesheet';
        link.href = 'markdown-katex-runtime.css';
        link.onload = function() {
            _mathStyleReady = true;
            scheduleHeightReport();
            resolve(true);
        };
        link.onerror = function(err) {
            console.warn('[JinMath] failed to load stylesheet', err);
            resolve(false);
        };
        document.head.appendChild(link);
    });
    return _mathStyleLoadPromise;
}

function loadMathRuntimeScripts() {
    if (window.katex && window.texmath) return Promise.resolve(true);
    return loadScriptOnce('markdown-katex-runtime.js', 'JinMath')
        .then(function(katexLoaded) {
            if (!katexLoaded) return false;
            return loadScriptOnce('markdown-texmath-runtime.js', 'JinMath');
        })
        .then(function(texmathLoaded) {
            return !!(texmathLoaded && window.katex && window.texmath);
        });
}

function ensureMathReady() {
    if (_mathReady && window.katex && window.texmath) {
        return Promise.resolve(true);
    }
    if (_mathRuntimeLoadPromise) {
        return _mathRuntimeLoadPromise;
    }

    _mathRuntimeLoadPromise = Promise.all([
        ensureMathStylesheet(),
        loadMathRuntimeScripts()
    ])
        .then(function(results) {
            var scriptReady = !!results[1];
            _mathReady = scriptReady;
            return scriptReady;
        })
        .catch(function() { return false; });

    return _mathRuntimeLoadPromise;
}

function loadMermaidRuntimeScript() {
    resolveMermaidFromGlobal();
    if (window.mermaid && typeof window.mermaid.render === 'function') {
        return Promise.resolve(true);
    }
    if (_mermaidRuntimeLoadPromise) {
        return _mermaidRuntimeLoadPromise;
    }
    _mermaidRuntimeLoadPromise = new Promise(function(resolve) {
        var script = document.createElement('script');
        script.src = 'markdown-mermaid-runtime.js';
        script.async = true;
        script.onload = function() {
            resolveMermaidFromGlobal();
            resolve(!!(window.mermaid && typeof window.mermaid.render === 'function'));
        };
        script.onerror = function(err) {
            console.warn('[JinMermaid] failed to load runtime script', err);
            resolve(false);
        };
        document.head.appendChild(script);
    });
    return _mermaidRuntimeLoadPromise;
}

function getMermaidRuntime() {
    resolveMermaidFromGlobal();
    if (window.mermaid && typeof window.mermaid.render === 'function') {
        return window.mermaid;
    }
    if (window.__esbuild_esm_mermaid_nm && window.__esbuild_esm_mermaid_nm.mermaid) {
        return window.__esbuild_esm_mermaid_nm.mermaid;
    }
    return null;
}

var copyIcon = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="5.5" y="5.5" width="8" height="8" rx="1.5"/><path d="M3.5 10.5h-1a1 1 0 0 1-1-1v-7a1 1 0 0 1 1-1h7a1 1 0 0 1 1 1v1"/></svg>';
var checkIcon = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3.5 8.5 6.5 11.5 12.5 4.5"/></svg>';
var renderIcon = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3.5 12.5 8 5 12.5 5 3.5"/></svg>';
var codeIcon = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 3.5 1.5 8 6 12.5"/><polyline points="10 3.5 14.5 8 10 12.5"/></svg>';
var foldIcon = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><polyline points="3.5 6 8 10 12.5 6"/></svg>';
var lineNumbersIcon = '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><line x1="1.5" y1="3.5" x2="3.5" y2="3.5"/><line x1="6" y1="3.5" x2="14.5" y2="3.5"/><line x1="1.5" y1="8" x2="3.5" y2="8"/><line x1="6" y1="8" x2="12" y2="8"/><line x1="1.5" y1="12.5" x2="3.5" y2="12.5"/><line x1="6" y1="12.5" x2="13.5" y2="12.5"/></svg>';
var expandIcon = '<svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><polyline points="4 6 8 10 12 6"/></svg>';

var _codeBlockSettings = { showLineNumbers: false, collapseLineThreshold: 25 };

function escapeHTML(text) {
    return String(text)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
        .replace(/'/g, '&#039;');
}

function escapeAttr(text) {
    return String(text)
        .replace(/&/g, '&amp;')
        .replace(/"/g, '&quot;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;');
}

var languageLogoAliases = {
    js: 'javascript',
    jsx: 'javascript',
    mjs: 'javascript',
    cjs: 'javascript',
    ts: 'typescript',
    tsx: 'typescript',
    py: 'python',
    rb: 'ruby',
    sh: 'bash',
    zsh: 'bash',
    shell: 'bash',
    shellscript: 'bash',
    shellsession: 'bash',
    console: 'bash',
    terminal: 'bash',
    fishshell: 'fish',
    nu: 'nushell',
    ps: 'powershell',
    pwsh: 'powershell',
    ps1: 'powershell',
    yml: 'yaml',
    md: 'markdown',
    txt: 'txt',
    plain: 'txt',
    plaintext: 'txt',
    text: 'txt',
    proto: 'protobuf',
    tex: 'latex',
    vuejs: 'vue',
    cplusplus: 'cpp',
    'c++': 'cpp',
    objc: 'objective-c',
    'obj-c': 'objective-c',
    'objective-cpp': 'cpp',
    mmd: 'mermaid',
    mermaid: 'mermaid',
    'mermaid-svg': 'mermaid',
    mermaidsvg: 'mermaid',
    cs: 'csharp',
    'c#': 'csharp',
    golang: 'go',
    jsx: 'javascript',
    tsx: 'typescript',
    shellscript: 'bash',
    sh: 'bash',
    zsh: 'bash',
    shell: 'bash',
    gdscript: 'godot',
    dockerfile: 'docker',
    scss: 'scss',
    sass: 'scss',
    dart: 'dart',
    julia: 'julia',
    crystal: 'crystal',
    fsharp: 'fsharp',
    'f#': 'fsharp',
    groovy: 'groovy',
    awk: 'awk',
    scheme: 'lisp',
    racket: 'racket',
    regex: 'regex',
    regexp: 'regex',
    make: 'make',
    makefile: 'make',
    brainfuck: 'brainfuck',
    fortran: 'fortran',
    'fortran-free-form': 'fortran',
    'fortran-fixed-form': 'fortran',
    mysql: 'sql',
    postgresql: 'sql',
    sqlite: 'sql',
    pgsql: 'sql',
    'angular-html': 'angular',
    'angular-ts': 'angular',
    'vue-html': 'vue',
    coffee: 'coffeescript',
    coffeescript: 'coffeescript',
    erb: 'ruby',
    blade: 'php',
    liquid: 'ruby',
    hcl: 'terraform',
    'ssh-config': 'bash',
    jsonc: 'json',
    json5: 'json',
    jsonl: 'json',
    mdx: 'markdown',
    rst: 'markdown',
    asciidoc: 'markdown',
    typst: 'latex',
    conf: 'ini',
    'system-verilog': 'verilog',
    verilog: 'verilog',
    vhdl: 'verilog',
    glsl: 'wgsl',
    hlsl: 'wgsl',
    viml: 'vim',
    vimscript: 'vim',
    'emacs-lisp': 'elisp',
    elisp: 'elisp',
    bat: 'bat',
    batch: 'bat',
    sml: 'sml',
    coq: 'coq',
    raku: 'raku',
    perl6: 'raku',
    hack: 'hack',
    lean: 'lean',
    lean4: 'lean',
    idris: 'idris',
    fennel: 'fennel',
    janet: 'janet',
    forth: 'forth',
    io: 'io',
    motoko: 'motoko',
    dhall: 'dhall',
    cue: 'cue',
    starlark: 'starlark',
    bazel: 'starlark',
    bicep: 'bicep',
    jsonnet: 'jsonnet',
    d: 'd',
    processing: 'processing',
    reasonml: 'reasonml',
    reason: 'reasonml',
    wat: 'wasm',
    solidity: 'solidity'
};

var languageLogoURLs = {
    bash: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bash/bash-original.svg',
    python: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg',
    javascript: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg',
    typescript: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg',
    swift: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/swift/swift-original.svg',
    go: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/go/go-original.svg',
    rust: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/rust/rust-original.svg',
    zig: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/zig/zig-original.svg',
    nim: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nim/nim-original.svg',
    ocaml: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/ocaml/ocaml-original.svg',
    haskell: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/haskell/haskell-original.svg',
    elm: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/elm/elm-original.svg',
    elixir: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/elixir/elixir-original.svg',
    erlang: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/erlang/erlang-original.svg',
    clojure: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/clojure/clojure-original.svg',
    lisp: 'https://api.iconify.design/simple-icons/commonlisp.svg?color=%23a38b6d',
    apl: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/apl/apl-original.svg',
    java: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg',
    kotlin: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/kotlin/kotlin-original.svg',
    v: 'https://api.iconify.design/simple-icons/v.svg?color=%235f6b77',
    c: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg',
    cpp: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg',
    csharp: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg',
    php: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg',
    ruby: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/ruby/ruby-original.svg',
    perl: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/perl/perl-original.svg',
    lua: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/lua/lua-original.svg',
    scala: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/scala/scala-original.svg',
    r: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/r/r-original.svg',
    prolog: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/prolog/prolog-original.svg',
    html: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg',
    css: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg',
    scss: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sass/sass-original.svg',
    postcss: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postcss/postcss-original.svg',
    xml: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/xml/xml-original.svg',
    graphql: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/graphql/graphql-plain.svg',
    svelte: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/svelte/svelte-original.svg',
    vue: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg',
    astro: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/astro/astro-original.svg',
    gleam: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/gleam/gleam-original.svg',
    nix: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nixos/nixos-original.svg',
    docker: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg',
    godot: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/godot/godot-original.svg',
    dart: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/dart/dart-original.svg',
    julia: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/julia/julia-original.svg',
    crystal: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/crystal/crystal-original.svg',
    fsharp: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fsharp/fsharp-original.svg',
    groovy: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/groovy/groovy-original.svg',
    wgsl: 'https://api.iconify.design/simple-icons/webgpu.svg?color=%235184fe',
    'objective-c': 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/objectivec/objectivec-plain.svg',
    solidity: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/solidity/solidity-original.svg',
    latex: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/latex/latex-original.svg',
    protobuf: 'https://api.iconify.design/simple-icons/protocolsdotio.svg?color=%23006A71',
    nushell: 'https://api.iconify.design/simple-icons/nushell.svg?color=%234e9a06',
    fish: 'https://api.iconify.design/simple-icons/fishshell.svg?color=%2334c534',
    powershell: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/powershell/powershell-original.svg',
    sql: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg',
    json: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg',
    yaml: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/yaml/yaml-original.svg',
    awk: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bash/bash-original.svg',
    tmux: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tmux/tmux-original.svg',
    regex: 'https://api.iconify.design/mdi/regex.svg?color=%2364748b',
    make: 'https://api.iconify.design/mdi/file-cog-outline.svg?color=%2364748b',
    brainfuck: 'https://api.iconify.design/mdi/console-line.svg?color=%2364748b',
    fortran: 'https://api.iconify.design/simple-icons/fortran.svg?color=%23734f96',
    angular: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/angular/angular-original.svg',
    coffeescript: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/coffeescript/coffeescript-original.svg',
    cmake: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cmake/cmake-original.svg',
    matlab: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/matlab/matlab-original.svg',
    nginx: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nginx/nginx-original.svg',
    purescript: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/purescript/purescript-original.svg',
    terraform: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/terraform/terraform-original.svg',
    handlebars: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/handlebars/handlebars-original.svg',
    less: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/less/less-plain-wordmark.svg',
    wasm: 'https://api.iconify.design/simple-icons/webassembly.svg?color=%23654ff0',
    toml: 'https://api.iconify.design/simple-icons/toml.svg?color=%239c4221',
    diff: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg',
    pascal: 'https://api.iconify.design/simple-icons/delphi.svg?color=%23ee1f35',
    racket: 'https://api.iconify.design/simple-icons/racket.svg?color=%239f1d20',
    odin: 'https://api.iconify.design/simple-icons/odin.svg?color=%2360798b',
    ada: 'https://api.iconify.design/simple-icons/ada.svg?color=%230d5ba6',
    cobol: 'https://api.iconify.design/mdi/code-braces.svg?color=%2300589c',
    wolfram: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mathematica/mathematica-original.svg',
    verilog: 'https://api.iconify.design/mdi/chip.svg?color=%23007bff',
    stylus: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/stylus/stylus-original.svg',
    pug: 'https://api.iconify.design/simple-icons/pug.svg?color=%23a86454',
    ini: 'https://api.iconify.design/mdi/file-settings-outline.svg?color=%2364748b',
    dotenv: 'https://api.iconify.design/mdi/file-key-outline.svg?color=%23ecd53f',
    csv: 'https://api.iconify.design/mdi/file-table-outline.svg?color=%2322863a',
    log: 'https://api.iconify.design/mdi/file-document-outline.svg?color=%2364748b',
    http: 'https://api.iconify.design/mdi/web.svg?color=%2364748b',
    tcl: 'https://api.iconify.design/mdi/code-tags.svg?color=%23e4790d',
    d: 'https://api.iconify.design/simple-icons/dlang.svg?color=%23b03030',
    vim: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vim/vim-original.svg',
    elisp: 'https://api.iconify.design/simple-icons/gnuemacs.svg?color=%237f5ab6',
    bat: 'https://api.iconify.design/mdi/console.svg?color=%2364748b',
    dos: 'https://api.iconify.design/mdi/console.svg?color=%2364748b',
    sml: 'https://api.iconify.design/mdi/lambda.svg?color=%2364748b',
    coq: 'https://api.iconify.design/mdi/function-variant.svg?color=%23d4a752',
    raku: 'https://api.iconify.design/simple-icons/raku.svg?color=%230298be',
    hack: 'https://api.iconify.design/simple-icons/hacklang.svg?color=%23878787',
    lean: 'https://api.iconify.design/mdi/lambda.svg?color=%23b30000',
    idris: 'https://api.iconify.design/mdi/lambda.svg?color=%23b30000',
    fennel: 'https://api.iconify.design/mdi/flower-outline.svg?color=%23fff3b5',
    janet: 'https://api.iconify.design/mdi/flower-tulip-outline.svg?color=%2369539e',
    forth: 'https://api.iconify.design/mdi/code-braces-box.svg?color=%23b03030',
    io: 'https://api.iconify.design/mdi/circle-outline.svg?color=%2364748b',
    motoko: 'https://api.iconify.design/simple-icons/dfinity.svg?color=%2329abe2',
    dhall: 'https://api.iconify.design/mdi/function-variant.svg?color=%236b8e23',
    cue: 'https://api.iconify.design/simple-icons/cue.svg?color=%233b82f6',
    starlark: 'https://api.iconify.design/simple-icons/bazel.svg?color=%2343a047',
    bicep: 'https://api.iconify.design/simple-icons/microsoftazure.svg?color=%230078d4',
    jsonnet: 'https://api.iconify.design/mdi/code-json.svg?color=%23f59e0b',
    processing: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/processing/processing-original.svg',
    zig: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/zig/zig-original.svg',
    reasonml: 'https://api.iconify.design/simple-icons/reason.svg?color=%23dd4b39',
    txt: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/markdown/markdown-original.svg',
    markdown: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/markdown/markdown-original.svg',
    mermaid: 'https://api.iconify.design/simple-icons/mermaid.svg?color=%23ff3670'
};

var languageHighlightAliases = {
    nu: 'nushell',
    console: 'shellsession',
    terminal: 'shellsession',
    shellscript: 'bash',
    shell: 'bash',
    zsh: 'bash',
    plain: 'txt',
    plaintext: 'txt',
    text: 'txt',
    fortran: 'fortran',
    assembly: 'asm',
    x86: 'asm',
    elisp: 'emacs-lisp',
    vim: 'viml',
    vimscript: 'viml',
    wat: 'wasm',
    perl6: 'raku',
    bazel: 'starlark',
    delphi: 'pascal'
};

function resolveLanguageLabel(lang) {
    var raw = lang ? String(lang).trim().toLowerCase() : '';
    if (!raw) return 'txt';
    if (window.JinPrism && typeof window.JinPrism.languageLabel === 'function') {
        return window.JinPrism.languageLabel(raw) || raw;
    }
    return languageHighlightAliases[raw] || raw;
}

function resolveLanguageLogoURL(lang, label) {
    var key = '';
    if (window.JinPrism && typeof window.JinPrism.languageLogoKey === 'function') {
        key = window.JinPrism.languageLogoKey(lang || label || '');
    }
    if (!key) {
        key = resolveLanguageLabel(lang || label || '');
        key = languageLogoAliases[key] || key;
    }
    return languageLogoURLs[key] || '';
}

function detectRenderableKind(label, code) {
    var lang = String(label || '').trim().toLowerCase();
    if (lang === 'mmd') lang = 'mermaid';
    if (lang === 'mermaid' || lang === 'mermaid-svg' || lang === 'mermaidsvg') return 'mermaid';
    if (lang === 'svg' && isLikelySVGMarkup(code)) return 'svg';
    if ((lang === 'xml' || lang === 'html' || lang === 'txt' || lang === 'plaintext') && isLikelySVGMarkup(code)) {
        return 'svg';
    }
    return '';
}

function isLikelySVGMarkup(code) {
    var text = String(code || '').trim();
    if (!text) return false;
    text = text.replace(/^<\?xml[\s\S]*?\?>\s*/i, '');
    return /^<svg[\s>]/i.test(text);
}

function codeHeaderHTML(id, label, lang, renderKind, isCollapsible, isHeightCollapsed) {
    var logoURL = resolveLanguageLogoURL(lang, label);
    var fallbackText = String(label || '').slice(0, 2) || 'txt';
    var logo = logoURL
        ? '<img class="lang-logo" src="' + escapeAttr(logoURL) + '" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display=&quot;none&quot;" />'
        : '<span class="lang-logo lang-logo-fallback">' + escapeHTML(fallbackText) + '</span>';
    var renderButton = renderKind
        ? '<button type="button" class="render-btn" id="render-' + id + '" title="Render preview" aria-label="Render preview" aria-controls="preview-' + id + '" aria-pressed="false" onclick="toggleCodeRender(' + id + ')">' + renderIcon + '</button>'
        : '';
    var lineNumbersActive = _codeBlockSettings.showLineNumbers;
    var lnClass = 'line-numbers-btn' + (lineNumbersActive ? ' is-active' : '');
    var lnPressed = lineNumbersActive ? 'true' : 'false';
    var lineNumbersButton = '<button type="button" class="' + lnClass + '" id="ln-' + id + '" title="Toggle line numbers" aria-label="Toggle line numbers" aria-pressed="' + lnPressed + '" onclick="toggleLineNumbers(' + id + ')">' + lineNumbersIcon + '</button>';
    var foldButton = '';
    if (isCollapsible) {
        var foldClass = 'fold-btn' + (isHeightCollapsed ? ' is-folded' : '');
        var foldTitle = isHeightCollapsed ? 'Show all lines' : 'Collapse code';
        foldButton = '<button type="button" class="' + foldClass + '" id="fold-' + id + '" title="' + foldTitle + '" aria-label="' + foldTitle + '" aria-expanded="' + (isHeightCollapsed ? 'false' : 'true') + '" onclick="toggleCodeHeightFold(' + id + ')">' + foldIcon + '</button>';
    }
    return '<div class="code-header"><span class="code-header-leading"><span class="lang-label">' + logo + '<span class="lang-text">' + escapeHTML(label) + '</span></span></span><span class="code-actions">' + renderButton + foldButton + lineNumbersButton + '<button type="button" class="copy-btn" id="copy-' + id + '" title="Copy code" aria-label="Copy code" onclick="copyCode(' + id + ')">' + copyIcon + '</button></span></div>';
}

function generateGutterHTML(lineCount) {
    var nums = [];
    for (var i = 1; i <= lineCount; i++) {
        nums.push(String(i));
    }
    return nums.join('\n');
}

function wrapCodeBlock(id, label, codeHTML, lang, renderKind, lineCount) {
    var preview = renderKind ? '<div class="code-preview" id="preview-' + id + '" hidden></div>' : '';
    var gutter = '<div class="line-numbers-gutter" id="gutter-' + id + '">' + generateGutterHTML(lineCount) + '</div>';
    var isCollapsible = _codeBlockDisplayMode === 'collapsible' && lineCount > _codeBlockCollapseLineThreshold;
    var wrapperClass = 'code-content-wrapper' + (isCollapsible ? ' is-height-collapsed' : '');
    var blockClass = 'code-block';
    if (isCollapsible) blockClass += ' is-height-collapsible';
    if (_codeBlockSettings.showLineNumbers) blockClass += ' show-line-numbers';
    var hiddenLines = isCollapsible ? (lineCount - _codeBlockCollapseLineThreshold) : 0;
    var expandLabel = hiddenLines > 0 ? hiddenLines + ' more lines' : 'Show more';
    var expandAria = hiddenLines > 0 ? 'Show ' + hiddenLines + ' more lines' : 'Show more';
    var expandBar = isCollapsible
        ? '<div class="code-expand-bar" id="expand-bar-' + id + '"><button type="button" class="code-expand-pill" id="expand-pill-' + id + '" title="' + expandAria + '" aria-label="' + expandAria + '" aria-controls="wrapper-' + id + '" aria-expanded="false" onclick="toggleCodeHeightFold(' + id + ')">' + expandIcon + ' <span id="expand-label-' + id + '">' + expandLabel + '</span></button></div>'
        : '';
    return '<div class="' + blockClass + '" id="code-block-' + id + '">' + codeHeaderHTML(id, label, lang, renderKind, isCollapsible, isCollapsible) + '<div class="' + wrapperClass + '" id="wrapper-' + id + '"><div class="code-content" id="content-' + id + '"><div class="code-raw" id="raw-' + id + '">' + gutter + codeHTML + '</div>' + preview + '</div></div>' + expandBar + '</div>';
}

function decodeBase64UTF8(base64) {
    var bytes = Uint8Array.from(atob(base64), function(c) { return c.charCodeAt(0); });
    return new TextDecoder('utf-8').decode(bytes);
}

function copyCode(id) {
    var text = _codeBlockSrc[id] || '';
    var btn = document.getElementById('copy-' + id);
    function onSuccess() {
        if (btn) { btn.innerHTML = checkIcon; btn.classList.add('copied'); setTimeout(function(){ btn.innerHTML = copyIcon; btn.classList.remove('copied'); }, 1500); }
    }
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.copyText) {
        window.webkit.messageHandlers.copyText.postMessage(text);
        onSuccess();
    } else if (navigator.clipboard) {
        navigator.clipboard.writeText(text).then(onSuccess);
    }
}

function abbreviateErrorText(text, maxLength) {
    var value = String(text || '').trim();
    if (!value) return '';
    var limit = typeof maxLength === 'number' && maxLength > 0 ? maxLength : 260;
    return value.length > limit ? value.slice(0, limit) + '…' : value;
}

function parseSVGRootFromMarkup(svgMarkup) {
    var parser = new DOMParser();
    var xmlDoc = parser.parseFromString(svgMarkup, 'image/svg+xml');
    var xmlError = xmlDoc && xmlDoc.querySelector ? xmlDoc.querySelector('parsererror') : null;

    if (!xmlError) {
        var xmlRoot = xmlDoc ? xmlDoc.documentElement : null;
        if (xmlRoot && String(xmlRoot.nodeName || '').toLowerCase() === 'svg') {
            return { ok: true, root: xmlRoot };
        }
        return { ok: false, error: 'SVG root element is missing or invalid.' };
    }

    var htmlDoc = parser.parseFromString(svgMarkup, 'text/html');
    var htmlRoot = htmlDoc && htmlDoc.querySelector ? htmlDoc.querySelector('svg') : null;
    if (htmlRoot) {
        var serializedHTMLRoot = new XMLSerializer().serializeToString(htmlRoot);
        var reparsedXML = parser.parseFromString(serializedHTMLRoot, 'image/svg+xml');
        var reparsedError = reparsedXML && reparsedXML.querySelector ? reparsedXML.querySelector('parsererror') : null;
        var reparsedRoot = reparsedXML ? reparsedXML.documentElement : null;
        if (!reparsedError && reparsedRoot && String(reparsedRoot.nodeName || '').toLowerCase() === 'svg') {
            return { ok: true, root: reparsedRoot };
        }
    }

    var parseErrorText = String(xmlError.textContent || xmlError.innerText || 'Unknown SVG parse error.');
    return { ok: false, error: 'SVG parse error: ' + abbreviateErrorText(parseErrorText, 320) };
}

function sanitizeSVGMarkupDetailed(svgMarkup, options) {
    var text = String(svgMarkup || '').trim();
    if (!text) return { ok: false, error: 'SVG content is empty.' };
    text = text.replace(/^<\?xml[\s\S]*?\?>\s*/i, '');
    var cjkFontFallback = 'PingFang SC, Hiragino Sans GB, Microsoft YaHei, Noto Sans CJK SC, Source Han Sans SC, Heiti SC, WenQuanYi Micro Hei, sans-serif';
    var allowForeignObject = !!(options && options.allowForeignObject);

    var parsed = parseSVGRootFromMarkup(text);
    if (!parsed.ok || !parsed.root) {
        return { ok: false, error: parsed.error || 'Unable to parse SVG content.' };
    }
    var root = parsed.root;

    var forbiddenTags = allowForeignObject
        ? ['script', 'iframe', 'object', 'embed', 'link', 'meta']
        : ['script', 'foreignObject', 'iframe', 'object', 'embed', 'link', 'meta'];
    forbiddenTags.forEach(function(tagName) {
        var nodes = root.querySelectorAll(tagName);
        nodes.forEach(function(node) { node.remove(); });
    });

    root.querySelectorAll('*').forEach(function(node) {
        Array.from(node.attributes || []).forEach(function(attr) {
            var name = String(attr.name || '').toLowerCase();
            var value = String(attr.value || '').trim();

            if (name.slice(0, 2) === 'on') {
                node.removeAttribute(attr.name);
                return;
            }

            if ((name === 'href' || name === 'xlink:href' || name === 'src' || name === 'data') && !isSafeSVGHref(value)) {
                node.removeAttribute(attr.name);
                return;
            }

            if (name === 'style' && /(?:expression\s*\(|url\s*\(\s*['"]?\s*(?:javascript:|data:text\/html))/i.test(value)) {
                node.removeAttribute(attr.name);
            }
        });
    });

    if (!root.getAttribute('xmlns')) {
        root.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    }
    if (!root.getAttribute('font-family')) {
        root.setAttribute('font-family', cjkFontFallback);
    }
    root.querySelectorAll('text, tspan').forEach(function(node) {
        if (!node.getAttribute('font-family')) {
            node.setAttribute('font-family', cjkFontFallback);
        }
    });
    if (allowForeignObject) {
        root.querySelectorAll('foreignObject, foreignObject *').forEach(function(node) {
            var styleValue = String(node.getAttribute('style') || '').trim();
            if (!/font-family\s*:/i.test(styleValue)) {
                var mergedStyle = styleValue ? styleValue.replace(/;?\s*$/, '; ') + 'font-family: ' + cjkFontFallback + ';' : 'font-family: ' + cjkFontFallback + ';';
                node.setAttribute('style', mergedStyle);
            }
        });
    }

    var serialized = new XMLSerializer().serializeToString(root);
    if (!serialized || !/^<svg[\s>]/i.test(serialized)) {
        return { ok: false, error: 'Sanitized output is not a valid <svg> element.' };
    }

    return { ok: true, svg: serialized };
}

function sanitizeSVGMarkup(svgMarkup) {
    var result = sanitizeSVGMarkupDetailed(svgMarkup);
    return result.ok ? result.svg : '';
}

function isSafeSVGHref(value) {
    if (!value) return true;
    var normalized = String(value).trim().toLowerCase();
    return normalized.charAt(0) === '#';
}

function renderPreviewErrorHTML(message) {
    return '<pre class="preview-error">' + escapeHTML(message || 'Render failed.') + '</pre>';
}

function ensureMermaidReady() {
    if (!_mermaidReadyPromise) {
        _mermaidReadyPromise = loadMermaidRuntimeScript().then(function(loaded) {
            if (!loaded) return false;
            var mermaidRuntime = getMermaidRuntime();
            if (!mermaidRuntime || typeof mermaidRuntime.render !== 'function') {
                return false;
            }
            if (!window.mermaid) {
                window.mermaid = mermaidRuntime;
            }
            mermaidRuntime.initialize({
                startOnLoad: false,
                securityLevel: 'strict',
                theme: 'base',
                flowchart: {
                    htmlLabels: false
                },
                themeVariables: {
                    fontFamily: 'PingFang SC, Hiragino Sans GB, Microsoft YaHei, Noto Sans CJK SC, Source Han Sans SC, Heiti SC, WenQuanYi Micro Hei, sans-serif'
                },
                deterministicIds: true,
                suppressErrorRendering: true
            });
            return true;
        }).catch(function(err) {
            console.warn('[JinMermaid] initialization failed', err);
            return false;
        });
    }
    return _mermaidReadyPromise;
}

async function renderCodePreview(kind, code, id) {
    if (kind === 'svg') {
        var safeSVGResult = sanitizeSVGMarkupDetailed(code);
        if (!safeSVGResult.ok) {
            return { ok: false, error: safeSVGResult.error || 'Invalid SVG markup.' };
        }
        return { ok: true, html: '<div class="diagram-preview svg-preview">' + safeSVGResult.svg + '</div>' };
    }

    if (kind === 'mermaid') {
        var ready = await ensureMermaidReady();
        if (!ready) return { ok: false, error: 'Mermaid runtime is unavailable.' };
        try {
            var mermaidRuntime = getMermaidRuntime();
            if (!mermaidRuntime || typeof mermaidRuntime.render !== 'function') {
                return { ok: false, error: 'Mermaid runtime is unavailable.' };
            }
            var renderID = 'jin-mermaid-' + id + '-' + Date.now();
            var result = await mermaidRuntime.render(renderID, code);
            var rawResultSVG = result && result.svg ? result.svg : '';
            if (!rawResultSVG) {
                return { ok: false, error: 'Mermaid returned empty SVG output.' };
            }
            var safeResult = sanitizeSVGMarkupDetailed(rawResultSVG, { allowForeignObject: true });
            if (!safeResult.ok) {
                return { ok: false, error: 'Mermaid SVG sanitize failed: ' + safeResult.error };
            }
            return {
                ok: true,
                html: '<div class="diagram-preview mermaid-preview">' + safeResult.svg + '</div>',
                bind: result && typeof result.bindFunctions === 'function' ? result.bindFunctions : null
            };
        } catch (err) {
            var errorMessage = '';
            if (err) {
                if (typeof err === 'string') {
                    errorMessage = err;
                } else if (typeof err.message === 'string' && err.message) {
                    errorMessage = err.message;
                } else if (typeof err.str === 'string' && err.str) {
                    errorMessage = err.str;
                } else {
                    try {
                        errorMessage = JSON.stringify(err);
                    } catch (_) {
                        errorMessage = String(err);
                    }
                }
            }
            return {
                ok: false,
                error: abbreviateErrorText(errorMessage || 'Unknown Mermaid render error.', 320)
            };
        }
    }

    return { ok: false, error: 'Unsupported render type.' };
}

function scheduleHeightReport() {
    setTimeout(reportHeight, 0);
}

function updateCodeHeightCollapseUI(id) {
    var state = _codeBlockState[id];
    if (!state) return;

    var block = document.getElementById('code-block-' + id);
    var wrapper = document.getElementById('wrapper-' + id);
    var foldButton = document.getElementById('fold-' + id);
    var expandPill = document.getElementById('expand-pill-' + id);
    var expandLabel = document.getElementById('expand-label-' + id);
    var isHeightCollapsible = !!state.heightCollapsible;

    if (block) {
        block.classList.toggle('is-height-collapsible', isHeightCollapsible);
    }
    if (wrapper) {
        wrapper.classList.toggle('is-height-collapsed', !!state.heightCollapsed);
    }

    var hiddenLines = Math.max(0, (state.lineCount || 0) - _codeBlockCollapseLineThreshold);
    if (foldButton) {
        if (state.heightCollapsed) {
            foldButton.classList.add('is-folded');
            foldButton.title = 'Show all lines';
            foldButton.setAttribute('aria-label', 'Show all lines');
            foldButton.setAttribute('aria-expanded', 'false');
        } else {
            foldButton.classList.remove('is-folded');
            foldButton.title = 'Collapse code';
            foldButton.setAttribute('aria-label', 'Collapse code');
            foldButton.setAttribute('aria-expanded', 'true');
        }
    }
    if (expandPill && expandLabel) {
        if (state.heightCollapsed) {
            expandPill.classList.remove('is-expanded');
            var showMoreAria = hiddenLines > 0 ? 'Show ' + hiddenLines + ' more lines' : 'Show more';
            expandPill.title = showMoreAria;
            expandPill.setAttribute('aria-label', showMoreAria);
            expandPill.setAttribute('aria-expanded', 'false');
            expandLabel.textContent = hiddenLines > 0 ? hiddenLines + ' more lines' : 'Show more';
        } else {
            expandPill.classList.add('is-expanded');
            expandPill.title = 'Show less';
            expandPill.setAttribute('aria-label', 'Show less');
            expandPill.setAttribute('aria-expanded', 'true');
            expandLabel.textContent = 'Show less';
        }
    }
}

function setCodeHeightCollapsed(id, collapsed) {
    var state = _codeBlockState[id];
    if (!state || !state.heightCollapsible) return;

    state.heightCollapsed = !!collapsed;
    updateCodeHeightCollapseUI(id);
}

function toggleCodeHeightFold(id) {
    var state = _codeBlockState[id];
    if (!state || !state.heightCollapsible) return;

    state.heightCollapsed = !state.heightCollapsed;
    updateCodeHeightCollapseUI(id);
    scheduleHeightReport();
}

function toggleLineNumbers(id) {
    var state = _codeBlockState[id];
    if (!state) return;

    var block = document.getElementById('code-block-' + id);
    var btn = document.getElementById('ln-' + id);
    if (!block) return;

    state.lineNumbers = !state.lineNumbers;
    block.classList.toggle('show-line-numbers', !!state.lineNumbers);

    if (btn) {
        btn.classList.toggle('is-active', !!state.lineNumbers);
        btn.setAttribute('aria-pressed', state.lineNumbers ? 'true' : 'false');
    }
    scheduleHeightReport();
}

function toggleCodeHeightCollapse(id) {
    toggleCodeHeightFold(id);
}

function setCodeBlockDisplayMode(mode) {
    _codeBlockDisplayMode = mode || 'expanded';
}

async function toggleCodeRender(id) {
    var state = _codeBlockState[id];
    if (!state || !state.renderKind) return;
    if (state.rendering) return;

    var raw = document.getElementById('raw-' + id);
    var preview = document.getElementById('preview-' + id);
    var renderButton = document.getElementById('render-' + id);
    if (!raw || !preview || !renderButton) return;

    if (state.previewing) {
        state.previewing = false;
        raw.hidden = false;
        preview.hidden = true;
        renderButton.classList.remove('is-active');
        renderButton.innerHTML = renderIcon;
        renderButton.title = 'Render preview';
        renderButton.setAttribute('aria-label', 'Render preview');
        renderButton.setAttribute('aria-pressed', 'false');
        if (state.previewAutoExpanded) {
            state.previewAutoExpanded = false;
            setCodeHeightCollapsed(id, true);
        }
        scheduleHeightReport();
        return;
    }

    state.previewAutoExpanded = false;
    if (state.heightCollapsible && state.heightCollapsed) {
        state.previewAutoExpanded = true;
        setCodeHeightCollapsed(id, false);
    }

    if (!state.previewRendered) {
        state.rendering = true;
        renderButton.classList.add('is-loading');
        renderButton.disabled = true;

        var rendered = await renderCodePreview(state.renderKind, _codeBlockSrc[id] || '', id);
        preview.innerHTML = rendered.ok ? rendered.html : renderPreviewErrorHTML(rendered.error);
        if (rendered.ok && typeof rendered.bind === 'function') {
            try {
                rendered.bind(preview);
            } catch (bindErr) {
                console.warn('[JinMermaid] bind functions failed', bindErr);
            }
        }
        state.previewRendered = true;

        state.rendering = false;
        renderButton.classList.remove('is-loading');
        renderButton.disabled = false;
    }

    state.previewing = true;
    raw.hidden = true;
    preview.hidden = false;
    renderButton.classList.add('is-active');
    renderButton.innerHTML = codeIcon;
    renderButton.title = 'Show raw code';
    renderButton.setAttribute('aria-label', 'Show raw code');
    renderButton.setAttribute('aria-pressed', 'true');
    scheduleHeightReport();
}

function ensurePrismReady() {
    if (_prismReady) {
        return Promise.resolve(true);
    }
    return loadPrismRuntimeScript()
        .catch(function() { return false; });
}

function addLanguageCandidate(candidates, value) {
    var text = value ? String(value).trim().toLowerCase() : '';
    if (!text) return;
    if (candidates.indexOf(text) === -1) {
        candidates.push(text);
    }
}

function buildHighlightLanguageCandidates(lang, label, renderKind) {
    var candidates = [];
    addLanguageCandidate(candidates, lang);
    addLanguageCandidate(candidates, label);
    addLanguageCandidate(candidates, languageHighlightAliases[String(lang || '').trim().toLowerCase()] || '');
    addLanguageCandidate(candidates, languageHighlightAliases[String(label || '').trim().toLowerCase()] || '');

    if (renderKind === 'svg') {
        addLanguageCandidate(candidates, 'svg');
        addLanguageCandidate(candidates, 'xml');
        addLanguageCandidate(candidates, 'html');
    } else if (renderKind === 'mermaid') {
        addLanguageCandidate(candidates, 'mermaid');
        addLanguageCandidate(candidates, 'markdown');
        addLanguageCandidate(candidates, 'md');
    }

    addLanguageCandidate(candidates, 'txt');
    return candidates;
}

function highlightCodeWithPrism(code, candidates) {
    if (!window.JinPrism || typeof window.JinPrism.highlightCode !== 'function') {
        return '';
    }
    for (var i = 0; i < candidates.length; i += 1) {
        var html = window.JinPrism.highlightCode(code, candidates[i]) || '';
        if (html) return html;
    }
    return '';
}

function codeLineCount(text) {
    if (!text) return 0;
    // Strip a single trailing newline — markdown-it fence tokens always
    // include one, but it doesn't represent a visible line.
    var s = text;
    if (s.charCodeAt(s.length - 1) === 10) s = s.slice(0, -1);
    if (!s) return 1;
    var count = 1;
    for (var i = 0; i < s.length; i += 1) {
        if (s.charCodeAt(i) === 10) count += 1;
    }
    return count;
}

function shouldAttemptCodeHighlight(code) {
    var text = String(code || '');
    if (!text) return true;
    if (text.length > MAX_HIGHLIGHT_CODE_LENGTH) return false;
    return codeLineCount(text) <= MAX_HIGHLIGHT_CODE_LINES;
}

function highlightCodeBlock(str, lang) {
    var id = ++_codeBlockId;
    _codeBlockSrc[id] = str;
    var label = resolveLanguageLabel(lang);
    var renderKind = detectRenderableKind(label, str);
    var lineCount = codeLineCount(str);
    var isCollapsible = _codeBlockDisplayMode === 'collapsible' && lineCount > _codeBlockCollapseLineThreshold;
    _codeBlockState[id] = {
        renderKind: renderKind,
        previewRendered: false,
        previewing: false,
        rendering: false,
        previewAutoExpanded: false,
        lineNumbers: !!_codeBlockSettings.showLineNumbers,
        heightCollapsible: isCollapsible,
        heightCollapsed: isCollapsible,
        lineCount: lineCount
    };

    var candidates = buildHighlightLanguageCandidates(lang, label, renderKind);
    var highlightedInner = '';
    if (shouldAttemptCodeHighlight(str)) {
        highlightedInner = highlightCodeWithPrism(str, candidates);
    }

    var codeHTML;
    if (highlightedInner) {
        codeHTML = '<pre><code class="prism-code">' + highlightedInner + '</code></pre>';
    } else {
        codeHTML = '<pre><code class="prism-code">' + escapeHTML(str) + '</code></pre>';
    }
    return wrapCodeBlock(id, label, codeHTML, lang, renderKind, lineCount);
}

function fenceLanguageFromInfo(info) {
    var text = info ? String(info).trim() : '';
    if (!text) return '';
    return text.split(/\s+/)[0] || '';
}

function installCodeBlockRenderer(instance) {
    instance.renderer.rules.fence = function(tokens, idx) {
        var token = tokens[idx];
        var lang = fenceLanguageFromInfo(token.info);
        return highlightCodeBlock(token.content || '', lang) + '\n';
    };

    instance.renderer.rules.code_block = function(tokens, idx) {
        var token = tokens[idx];
        return highlightCodeBlock(token.content || '', '') + '\n';
    };
}

function createMarkdownParserState() {
    var instance = window.markdownit({
        html: false,
        linkify: true,
        typographer: false
    });
    installCodeBlockRenderer(instance);
    return {
        instance: instance,
        mathInstalled: false
    };
}

function installMathPluginIfPossible(parserState) {
    if (!parserState || parserState.mathInstalled) return parserState && parserState.mathInstalled;
    if (!window.texmath || !window.katex || typeof parserState.instance.use !== 'function') {
        return false;
    }
    parserState.instance.use(window.texmath, {
        engine: window.katex,
        delimiters: ['dollars', 'brackets', 'beg_end'],
        katexOptions: { throwOnError: false, strict: false, trust: true }
    });
    parserState.mathInstalled = true;
    return true;
}

function containsLikelyCodeBlocks(markdownText) {
    var text = String(markdownText || '');
    if (!text) return false;
    if (/(^|\n)(?: {0,3}(?:```|~~~))/.test(text)) return true;
    return /(^|\n)(?: {4}|\t)\S/.test(text);
}

function containsLikelyMath(markdownText) {
    var text = String(markdownText || '');
    if (!text) return false;
    if (text.indexOf('\\') === -1 && text.indexOf('$') === -1) return false;
    if (/\\begin\{[a-zA-Z*]+\}/.test(text)) return true;
    if (/\\\((?:[\s\S]*?)\\\)/.test(text)) return true;
    if (/\$\$[\s\S]+?\$\$/.test(text)) return true;
    return /(^|[^\\])\$(?:[^$\n\\]|\\.)+?\$(?!\$)/.test(text);
}

function shouldDeferCodeHighlightUpgrade(options) {
    if (!options || typeof options !== 'object') return false;
    return options.deferCodeHighlightUpgrade === true;
}

function deferredHighlightDelayMs(markdownText) {
    var text = String(markdownText || '');
    if (!text) return 1200;
    var step = Math.max(1, Math.floor(text.length / 64));
    var hash = 0;
    for (var i = 0; i < text.length; i += step) {
        hash = ((hash << 5) - hash + text.charCodeAt(i)) | 0;
    }
    return 1200 + (Math.abs(hash) % 3200);
}

function renderMarkdownContent(parserState, text) {
    _codeBlockId = 0;
    _codeBlockSrc = {};
    _codeBlockState = {};
    document.getElementById('content').innerHTML = parserState.instance.render(text);
    applyPersistedHighlights();
    scheduleSelectionSnapshotPost();
    reportHeight();
}

function renderPlainTextContent(text) {
    _codeBlockId = 0;
    _codeBlockSrc = {};
    _codeBlockState = {};
    var content = document.getElementById('content');
    if (!content) return;
    content.innerHTML = '';
    var container = document.createElement('div');
    container.className = 'plain-text-content';
    container.textContent = String(text || '');
    content.appendChild(container);
    applyPersistedHighlights();
    scheduleSelectionSnapshotPost();
    reportHeight();
}

var mdState = createMarkdownParserState();

async function updateContent(base64, options) {
    var ticket = ++_stableRenderTicket;
    _latestStableBase64 = base64;
    var text = decodeBase64UTF8(base64);
    if (options && options.renderPlainText === true) {
        _latestStableText = text;
        renderPlainTextContent(text);
        return;
    }
    var needsCodeHighlight = containsLikelyCodeBlocks(text);
    var needsMath = containsLikelyMath(text);
    var deferCodeHighlightUpgrade = shouldDeferCodeHighlightUpgrade(options);

    if (needsMath && _mathReady) {
        installMathPluginIfPossible(mdState);
    }

    renderMarkdownContent(mdState, text);

    var rerenderQueued = false;
    function queueRerenderIfCurrent() {
        if (rerenderQueued) return;
        rerenderQueued = true;
        setTimeout(function() {
            rerenderQueued = false;
            if (ticket !== _stableRenderTicket || base64 !== _latestStableBase64) {
                return;
            }
            renderMarkdownContent(mdState, text);
        }, 0);
    }

    if (needsMath && !mdState.mathInstalled) {
        ensureMathReady().then(function(ok) {
            if (!ok) return;
            if (!installMathPluginIfPossible(mdState)) return;
            queueRerenderIfCurrent();
        });
    }

    if (needsCodeHighlight && !_prismReady) {
        var requestHighlightUpgrade = function() {
            if (ticket !== _stableRenderTicket || base64 !== _latestStableBase64) {
                return;
            }
            ensurePrismReady().then(function(ok) {
                if (!ok) return;
                queueRerenderIfCurrent();
            });
        };

        if (deferCodeHighlightUpgrade) {
            setTimeout(requestHighlightUpgrade, deferredHighlightDelayMs(text));
        } else {
            requestHighlightUpgrade();
        }
    }
}

async function updateContentText(text, options) {
    var ticket = ++_stableRenderTicket;
    _latestStableText = String(text || '');
    var renderedText = _latestStableText;
    if (options && options.renderPlainText === true) {
        renderPlainTextContent(renderedText);
        return;
    }
    var needsCodeHighlight = containsLikelyCodeBlocks(renderedText);
    var needsMath = containsLikelyMath(renderedText);
    var deferCodeHighlightUpgrade = shouldDeferCodeHighlightUpgrade(options);

    if (needsMath && _mathReady) {
        installMathPluginIfPossible(mdState);
    }

    renderMarkdownContent(mdState, renderedText);

    var rerenderQueued = false;
    function queueRerenderIfCurrent() {
        if (rerenderQueued) return;
        rerenderQueued = true;
        setTimeout(function() {
            rerenderQueued = false;
            if (ticket !== _stableRenderTicket || renderedText !== _latestStableText) {
                return;
            }
            renderMarkdownContent(mdState, renderedText);
        }, 0);
    }

    if (needsMath && !mdState.mathInstalled) {
        ensureMathReady().then(function(ok) {
            if (!ok) return;
            if (!installMathPluginIfPossible(mdState)) return;
            queueRerenderIfCurrent();
        });
    }

    if (needsCodeHighlight && !_prismReady) {
        var requestHighlightUpgrade = function() {
            if (ticket !== _stableRenderTicket || renderedText !== _latestStableText) {
                return;
            }
            ensurePrismReady().then(function(ok) {
                if (!ok) return;
                queueRerenderIfCurrent();
            });
        };

        if (deferCodeHighlightUpgrade) {
            setTimeout(requestHighlightUpgrade, deferredHighlightDelayMs(renderedText));
        } else {
            requestHighlightUpgrade();
        }
    }
}

function measuredHeight() {
    var el = document.getElementById('content');
    if (!el) {
        return Math.max(Math.ceil(document.body ? document.body.scrollHeight : 0), 1);
    }

    var rect = Math.ceil(el.getBoundingClientRect().height || 0);
    var scroll = Math.ceil(el.scrollHeight || 0);
    var offset = Math.ceil(el.offsetHeight || 0);

    return Math.max(rect, scroll, offset, 1) + 2;
}

function reportHeight() {
    var h = measuredHeight();
    _rh = h;
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.heightChanged) {
        window.webkit.messageHandlers.heightChanged.postMessage(h);
    }
}

var _rh = 0;
var resizeObserver = new ResizeObserver(function() {
    var h = measuredHeight();
    if (h !== _rh) { reportHeight(); }
});
var contentEl = document.getElementById('content');
if (contentEl) {
    resizeObserver.observe(contentEl);
}
resizeObserver.observe(document.body);
document.addEventListener('selectionchange', scheduleSelectionSnapshotPost, true);
document.addEventListener('mouseup', scheduleSelectionSnapshotPost, true);
document.addEventListener('contextmenu', scheduleSelectionSnapshotPost, true);

window.applyCodeBlockSettings = function(settings) {
    if (!settings) return;
    _codeBlockSettings.showLineNumbers = !!settings.showLineNumbers;
    var newThreshold = parseInt(settings.collapseLineThreshold, 10);
    if (newThreshold > 0) {
        _codeBlockSettings.collapseLineThreshold = newThreshold;
        _codeBlockCollapseLineThreshold = newThreshold;
        document.documentElement.style.setProperty('--code-collapse-height', (_codeBlockCollapseLineThreshold * 19.2 + 16) + 'px');
    }
    var blocks = document.querySelectorAll('.code-block');
    for (var i = 0; i < blocks.length; i++) {
        var block = blocks[i];
        var blockId = block.id.replace('code-block-', '');
        var state = _codeBlockState[blockId];
        block.classList.toggle('show-line-numbers', _codeBlockSettings.showLineNumbers);
        if (state) {
            state.lineNumbers = _codeBlockSettings.showLineNumbers;
            var wasCollapsible = state.heightCollapsible;
            state.heightCollapsible = _codeBlockDisplayMode === 'collapsible' && (state.lineCount || 0) > _codeBlockCollapseLineThreshold;
            if (state.heightCollapsible && !wasCollapsible) {
                state.heightCollapsed = true;
            } else if (!state.heightCollapsible) {
                state.heightCollapsed = false;
            }
        }
        var btn = document.getElementById('ln-' + blockId);
        if (btn) {
            btn.classList.toggle('is-active', _codeBlockSettings.showLineNumbers);
            btn.setAttribute('aria-pressed', _codeBlockSettings.showLineNumbers ? 'true' : 'false');
        }
        updateCodeHeightCollapseUI(blockId);
    }
    scheduleHeightReport();
};

window.setSelectionContext = setSelectionContext;
window.setPersistedHighlights = setPersistedHighlights;
window.updateWithBase64 = updateContent;
window.updateWithText = updateContentText;

// Streaming renderer uses the same Prism code block rendering as stable mode.
var streamMdState = createMarkdownParserState();

async function updateStreamingContent(base64, options) {
    var ticket = ++_streamRenderTicket;
    _latestStreamBase64 = base64;
    var text = decodeBase64UTF8(base64);
    var needsCodeHighlight = containsLikelyCodeBlocks(text);
    var needsMath = containsLikelyMath(text);
    var deferCodeHighlightUpgrade = shouldDeferCodeHighlightUpgrade(options);

    if (needsMath && _mathReady) {
        installMathPluginIfPossible(streamMdState);
    }

    renderMarkdownContent(streamMdState, text);

    var rerenderQueued = false;
    function queueRerenderIfCurrent() {
        if (rerenderQueued) return;
        rerenderQueued = true;
        setTimeout(function() {
            rerenderQueued = false;
            if (ticket !== _streamRenderTicket || base64 !== _latestStreamBase64) {
                return;
            }
            renderMarkdownContent(streamMdState, text);
        }, 0);
    }

    if (needsMath && !streamMdState.mathInstalled) {
        ensureMathReady().then(function(ok) {
            if (!ok) return;
            if (!installMathPluginIfPossible(streamMdState)) return;
            queueRerenderIfCurrent();
        });
    }

    if (needsCodeHighlight && !_prismReady) {
        var requestHighlightUpgrade = function() {
            if (ticket !== _streamRenderTicket || base64 !== _latestStreamBase64) {
                return;
            }
            ensurePrismReady().then(function(ok) {
                if (!ok) return;
                queueRerenderIfCurrent();
            });
        };

        if (deferCodeHighlightUpgrade) {
            setTimeout(requestHighlightUpgrade, deferredHighlightDelayMs(text));
        } else {
            requestHighlightUpgrade();
        }
    }
}

window.updateStreamingWithBase64 = updateStreamingContent;

async function updateStreamingContentText(text, options) {
    var ticket = ++_streamRenderTicket;
    _latestStreamText = String(text || '');
    var renderedText = _latestStreamText;
    var needsCodeHighlight = containsLikelyCodeBlocks(renderedText);
    var needsMath = containsLikelyMath(renderedText);
    var deferCodeHighlightUpgrade = shouldDeferCodeHighlightUpgrade(options);

    if (needsMath && _mathReady) {
        installMathPluginIfPossible(streamMdState);
    }

    renderMarkdownContent(streamMdState, renderedText);

    var rerenderQueued = false;
    function queueRerenderIfCurrent() {
        if (rerenderQueued) return;
        rerenderQueued = true;
        setTimeout(function() {
            rerenderQueued = false;
            if (ticket !== _streamRenderTicket || renderedText !== _latestStreamText) {
                return;
            }
            renderMarkdownContent(streamMdState, renderedText);
        }, 0);
    }

    if (needsMath && !streamMdState.mathInstalled) {
        ensureMathReady().then(function(ok) {
            if (!ok) return;
            if (!installMathPluginIfPossible(streamMdState)) return;
            queueRerenderIfCurrent();
        });
    }

    if (needsCodeHighlight && !_prismReady) {
        var requestHighlightUpgrade = function() {
            if (ticket !== _streamRenderTicket || renderedText !== _latestStreamText) {
                return;
            }
            ensurePrismReady().then(function(ok) {
                if (!ok) return;
                queueRerenderIfCurrent();
            });
        };

        if (deferCodeHighlightUpgrade) {
            setTimeout(requestHighlightUpgrade, deferredHighlightDelayMs(renderedText));
        } else {
            requestHighlightUpgrade();
        }
    }
}

window.updateStreamingWithText = updateStreamingContentText;
