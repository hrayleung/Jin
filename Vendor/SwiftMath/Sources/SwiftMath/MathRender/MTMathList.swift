...
public enum MTMathAtomType: Int, CustomStringConvertible, Comparable {
    ...
    case accent
    /// Content stacked above and/or below a base (\overset/\underset/\stackrel). Jin fork.
    case overUnderSet
    /// A boxed atom (\boxed): content with a stroked frame. Jin fork.
    case box
    ...
    public var description: String {
        switch self {
            ...
            case .overUnderSet:   return "OverUnderSet"
            case .box:            return "Box"
...
public class MTMathAtom: NSObject {
    ...
    public func copy() -> MTMathAtom {
        switch self.type {
            ...
            case .overUnderSet:
                return MTOverUnderSet(self as? MTOverUnderSet)
            case .box:
                return MTMathBox(self as? MTMathBox)
            case .space:
...
// MARK: - MTOverUnderSet
/** An atom with content stacked above and/or below a base, used by `\overset`,
    `\underset`, and `\stackrel` (Jin fork). `innerList` is the base; `over`
    and/or `under` are the stacked content. */
public class MTOverUnderSet: MTMathAtom {
    public var over: MTMathList?
    public var under: MTMathList?
    public var innerList: MTMathList?

    init(_ set: MTOverUnderSet?) {
        super.init(set)
        self.type = .overUnderSet
        if let set {
            if let o = set.over { self.over = MTMathList(o) }
            if let u = set.under { self.under = MTMathList(u) }
            if let i = set.innerList { self.innerList = MTMathList(i) }
        }
    }
    ...
    override public var finalized: MTMathAtom {
        let new = super.finalized as! MTOverUnderSet
        new.over = new.over?.finalized
        new.under = new.under?.finalized
        new.innerList = new.innerList?.finalized
        return new
    }
}

// MARK: - MTMathBox
/** A `\boxed{…}` atom: the inner list drawn with a stroked frame (Jin fork). */
public class MTMathBox: MTMathAtom {
    public var innerList: MTMathList?

    init(_ box: MTMathBox?) {
        super.init(box)
        self.type = .box
        if let box, let i = box.innerList { self.innerList = MTMathList(i) }
    }
    ...
    override public var finalized: MTMathAtom {
        let new = super.finalized as! MTMathBox
        new.innerList = new.innerList?.finalized
        return new
    }
}
