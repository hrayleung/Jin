...
public class MTMathImage {
    ...
    // Jin fork addition: expose the rendered baseline metrics so callers can
    // align the produced image on a text baseline (e.g. as an NSTextAttachment
    // for inline math). Populated by `asImage()`.
