...
func getInterElementSpaceArrayIndexForType(_ type:MTMathAtomType, row:Bool) -> Int {
    switch type {
        ...
        case .number, .variable, .unaryOperator:
            return 0
        // Decorative types (accent, underline, overline, overUnderSet, box) are treated as ordinary
        case .accent, .underline, .overline, .overUnderSet, .box:
            return 0
...
class MTTypesetter {
    ...
    // \overset/\underset/\stackrel (Jin fork): stack `over`/`under` (script size)
    // above/below the base, reusing the large-operator limits stacker. Unlike
    // \sum limits there is NO display/text style gate — always stack.
    func makeOverUnderSet(_ set: MTOverUnderSet) -> MTDisplay? {
        guard let baseDisplay = MTTypesetter.createLineForMathList(set.innerList, font: font, style: style, cramped: cramped) else {
            return nil
        }
        let sStyle = self.scriptStyle()
        let scriptFont = font.copy(withSize: MTTypesetter.getStyleSize(sStyle, font: font))

        var over: MTMathListDisplay? = nil
        var under: MTMathListDisplay? = nil
        if set.over != nil {
            over = MTTypesetter.createLineForMathList(set.over, font: scriptFont, style: sStyle, cramped: self.superScriptCramped())
        }
        if set.under != nil {
            under = MTTypesetter.createLineForMathList(set.under, font: scriptFont, style: sStyle, cramped: self.subscriptCramped())
        }

        let display = MTLargeOpLimitsDisplay(withNucleus: baseDisplay, upperLimit: over, lowerLimit: under, limitShift: 0, extraPadding: 0)
        if let over {
            display.upperLimitGap = max(styleFont.mathTable!.upperLimitGapMin, styleFont.mathTable!.upperLimitBaselineRiseMin - over.descent)
        }
        if let under {
            display.lowerLimitGap = max(styleFont.mathTable!.lowerLimitGapMin, styleFont.mathTable!.lowerLimitBaselineDropMin - under.ascent)
        }
        display.position = currentPosition
        display.range = set.indexRange
        return display
    }

    // \boxed{…} (Jin fork): inner content framed by a stroked rectangle, with
    // padding baked into the metrics so neighbours don't overlap the frame.
    func makeBox(_ box: MTMathBox) -> MTDisplay? {
        guard let inner = MTTypesetter.createLineForMathList(box.innerList, font: font, style: style, cramped: cramped) else {
            return nil
        }
        let padding = 4 * styleFont.mathTable!.muUnit
        let lineThickness = styleFont.mathTable!.underbarRuleThickness
        let display = MTBoxDisplay(withInner: inner, padding: padding, lineThickness: lineThickness, range: box.indexRange)
        display.position = currentPosition
        return display
    }
