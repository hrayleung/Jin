...
/// Rendering of a `\boxed{…}` — the inner list with padding and a stroked
/// rectangle around it (Jin fork). Padding is baked into ascent/descent/width
/// by the typesetter; the position setter cascades to the inner display.
class MTBoxDisplay : MTDisplay {
    var inner: MTMathListDisplay?
    var padding: CGFloat = 0
    var lineThickness: CGFloat = 0

    init(withInner inner: MTMathListDisplay?, padding: CGFloat, lineThickness: CGFloat, range: NSRange) {
        super.init()
        self.inner = inner
        self.padding = padding
        self.lineThickness = lineThickness
        self.width = (inner?.width ?? 0) + 2 * padding
        self.ascent = (inner?.ascent ?? 0) + padding
        self.descent = (inner?.descent ?? 0) + padding
        self.range = range
    }
    ...
    func updateInnerPosition() {
        // Shift the inner content right by the padding; baseline stays aligned,
        // so the baked vertical padding sits above/below the content.
        inner?.position = CGPointMake(self.position.x + padding, self.position.y)
    }
    ...
    override func draw(_ context: CGContext) {
        ...
        context.saveGState()
        self.textColor?.setStroke()
        let rect = self.displayBounds().insetBy(dx: lineThickness / 2, dy: lineThickness / 2)
