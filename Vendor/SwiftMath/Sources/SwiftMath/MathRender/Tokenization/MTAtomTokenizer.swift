...
class MTAtomTokenizer {
    ...
    private func tokenizeAtom(_ atom: MTMathAtom, prevAtom: MTMathAtom?, atomIndex: Int, allAtoms: [MTMathAtom]) -> MTBreakableElement? {
        ...
        case .overUnderSet:
            return tokenizeOverUnderSet(atom as! MTOverUnderSet, prevAtom: prevAtom, atomIndex: atomIndex)

        case .box:
            return tokenizeBox(atom as! MTMathBox, prevAtom: prevAtom, atomIndex: atomIndex)
    ...
    private func tokenizeOverUnderSet(_ atom: MTOverUnderSet, prevAtom: MTMathAtom?, atomIndex: Int) -> MTBreakableElement? {
        let typesetter = MTTypesetter(withFont: font, style: style, cramped: cramped, spaced: false)
        guard let display = typesetter.makeOverUnderSet(atom) else { return nil }
    ...
    private func tokenizeBox(_ atom: MTMathBox, prevAtom: MTMathAtom?, atomIndex: Int) -> MTBreakableElement? {
        let typesetter = MTTypesetter(withFont: font, style: style, cramped: cramped, spaced: false)
        guard let display = typesetter.makeBox(atom) else { return nil }
