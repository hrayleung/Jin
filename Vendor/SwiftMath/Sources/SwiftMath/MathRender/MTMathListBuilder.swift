...
public struct MTMathListBuilder {
    ...
    mutating func atomForCommand(_ command:String) -> MTMathAtom? {
        ...
        if let accent = MTMathAtomFactory.accent(withName: command) {
            ...
            // \overset{top}{base} / \stackrel{top}{base}: top stacked above base.
            let set = MTOverUnderSet()
            set.over = self.buildInternal(true)
            set.innerList = self.buildInternal(true)
            return set
        } else if command == "underset" {
            // \underset{bottom}{base}: bottom stacked below base.
            let set = MTOverUnderSet()
            set.under = self.buildInternal(true)
            set.innerList = self.buildInternal(true)
            return set
        } else if command == "boxed" {
            // \boxed{body}: body framed by a stroked rectangle.
            let box = MTMathBox()
    ...
    mutating func readCommand() -> String {
        // `:` added (Jin fork) so `\:` parses as a single-char command → a 4mu
        // medium space (see supportedLatexSymbols), matching `\>`.
        let singleChars = "{}$#%_| ,>;!\\:"
        if self.hasCharacters {
            let char = self.getNextCharacter()
